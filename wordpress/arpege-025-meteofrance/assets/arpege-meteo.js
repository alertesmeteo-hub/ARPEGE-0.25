(function () {
    ...
    function temperatureClass(value) {
        ...
        if (value >= 35) { return 'arp025-temp-extreme'; }
        if (value >= 30) { return 'arp025-temp-hot'; }
        if (value >= 22) { return 'arp025-temp-warm'; }
        if (value >= 12) { return 'arp025-temp-mild'; }
        if (value >= 4) { return 'arp025-temp-cool'; }
        return 'arp025-temp-cold';
    }

    function gustClass(value) {
        if (!finite(value)) { return ''; }
        if (value >= 80) { return 'arp025-gust-danger'; }
        if (value >= 60) { return 'arp025-gust-strong'; }
        if (value >= 40) { return 'arp025-gust-moderate'; }
        return '';
    }

    function windForceClass(value) {
        if (!finite(value)) { return 'arp025-wind-calm'; }
        if (value >= 80) { return 'arp025-wind-violent'; }
        if (value >= 60) { return 'arp025-wind-strong'; }
        if (value >= 40) { return 'arp025-wind-fresh'; }
        if (value >= 20) { return 'arp025-wind-moderate'; }
        return 'arp025-wind-light';
    }

    function rainClass(value) {
        if (!finite(value) || value < 0.1) { return ''; }
        if (value >= 15) { return 'arp025-rain arp025-rain-intense'; }
        if (value >= 5) { return 'arp025-rain arp025-rain-strong'; }
        if (value >= 2) { return 'arp025-rain arp025-rain-moderate'; }
        if (value >= 0.5) { return 'arp025-rain arp025-rain-light'; }
        return 'arp025-rain arp025-rain-trace';
    }
    ...
    function chartDayBands(svg, count, margin, innerWidth, innerHeight, meta) {
        ...
        while (start < count) {
            ...
            svg.appendChild(svgElement('rect', {
                ...
                height: innerHeight,
                class: 'arp025-chart-day-band arp025-dayband-' + token
            }));
            ...
            svg.appendChild(svgElement('rect', {
                ...
                width: Math.max(0, endX - startX),
                height: 46,
                class: 'arp025-chart-day-strip arp025-daystrip-' + token
            }));
            ...
            if (start > 0) {
                svg.appendChild(svgElement('line', {
                    ...
                    class: 'arp025-chart-day-separator arp025-dayseparator-' + token
                }));
            }
            ...
            if (endX - startX > 72) {
                ...
                svg.appendChild(svgElement('text', {
                    ...
                    y: margin.top + 31,
                    'text-anchor': 'middle',
                    class: 'arp025-chart-day-label arp025-daylabel-' + token
                }, dayLabel));
            }
            start = end + 1;
        }
    ...
    function appendChartMetrics(container, metrics) {
        ...
        block.className = 'arp025-chart-metrics';
        valid.forEach(function (metric) {
            var item = document.createElement('span');
            item.className = 'arp025-chart-metric';
            item.appendChild(textSpan(metric.label, 'arp025-chart-metric-label'));
            item.appendChild(textSpan(metric.value, 'arp025-chart-metric-value'));
    ...
    function appendChartViewport(container, svg) {
        ...
        viewport.className = 'arp025-chart-viewport';
        var tooltip = document.createElement('div');
        tooltip.className = 'arp025-chart-tooltip';
        tooltip.hidden = true;
        ...
        function tooltipTarget(node) {
            ...
            return node.closest('[data-arp025-tooltip]');
        }
        ...
        viewport.addEventListener('pointerover', function (event) {
            ...
            if (!target) { return; }
            tooltip.textContent = target.getAttribute('data-arp025-tooltip') || '';
    ...
    function drawExtremaLabels(svg, points, values, formatter, className) {
        ...
        indexes.forEach(function (index) {
            ...
            svg.appendChild(svgElement('text', {
                ...
                y: y,
                'text-anchor': 'middle',
                class: 'arp025-chart-value-label ' + className
    ...
    function renderLineChart(container, labels, series, options) {
        ...
        if (!scale) {
            container.appendChild(textSpan('Données indisponibles pour ce diagramme.', 'arp025-chart-empty'));
            return;
        }
        ...
        svg.classList.add('arp025-chart-svg');
        ...
        for (var g = 0; g <= tickValue; g += 1) {
            ...
            svg.appendChild(svgElement('line', {
                ...
                y1: gy,
                y2: gy,
                class: 'arp025-chart-grid'
            }));
            svg.appendChild(svgElement('text', {
                x: margin.left - 9,
                y: gy + 4,
                'text-anchor': 'end',
                class: 'arp025-chart-axis'
        ...
        chartTickIndexes(n, 8).forEach(function (index) {
            ...
            svg.appendChild(svgElement('line', {
                ...
                y1: margin.top,
                y2: margin.top + innerHeight,
                class: 'arp025-chart-grid arp025-chart-grid-vertical'
            }));
            svg.appendChild(svgElement('text', {
                x: x,
                y: height - 18,
                'text-anchor': index === 0 ? 'start' : (index === n - 1 ? 'end' : 'middle'),
                class: 'arp025-chart-axis arp025-chart-xaxis'
            }, axisLabel));
        });
        ...
        series.forEach(function (item, seriesIndex) {
            ...
            if (series.length === 1 && options.fillArea) {
                segments.forEach(function (segment) {
                    ...
                    svg.appendChild(svgElement('path', {
                        d: d,
                        class: 'arp025-chart-area ' + item.className
                    }));
                });
            }
            ...
            segments.forEach(function (segment) {
                svg.appendChild(svgElement('path', {
                    ...
                    class: 'arp025-chart-line ' + item.className,
                    fill: 'none'
                }));
            });
            ...
            points.forEach(function (point, index) {
                ...
                var attrs = {
                    ...
                    class: (options.showPoints === false ? 'arp025-chart-hitpoint ' : 'arp025-chart-point ') + item.className,
                    tabindex: 0,
                    'aria-label': tooltipLabel,
                    'data-arp025-tooltip': tooltipLabel
                };
        ...
        if (options.bandBetween && seriesPoints.length >= 2) {
            ...
            if (upperPoints.length > 1 && upperPoints.length === lowerPoints.length) {
                ...
                var envelope = svgElement('path', {
                    d: area,
                    class: 'arp025-chart-wind-envelope'
                });
                var firstLine = svg.querySelector('.arp025-chart-line');
        ...
        if (series.length > 1) {
            var legend = svgElement('g', { class: 'arp025-chart-legend' });
            series.forEach(function (item, index) {
                ...
                legend.appendChild(svgElement('text', {
                    x: x + 36,
                    y: 21,
                    class: 'arp025-chart-axis arp025-chart-legend-text'
                }, item.label));
            });
    ...
    function rainVisualClass(value) {
        if (!finite(value) || value < 0.1) { return 'arp025-chart-rain-zero'; }
        if (value >= 15) { return 'arp025-chart-rain-intense'; }
        if (value >= 5) { return 'arp025-chart-rain-strong'; }
        if (value >= 2) { return 'arp025-chart-rain-moderate'; }
        if (value >= 0.5) { return 'arp025-chart-rain-light'; }
        return 'arp025-chart-rain-trace';
    }
    ...
    function renderRainChart(container, labels, rainValues, cumulativeValues, options) {
        ...
        for (var cg = 0; cg <= cumTickCount; cg += 1) {
            ...
            svg.appendChild(svgElement('text', {
                ...
                y: cy + 5,
                'text-anchor': 'start',
                class: 'arp025-chart-axis arp025-chart-axis-cumulative'
        ...
        rainValues.forEach(function (value, index) {
            ...
            var bar = svgElement('rect', {
                ...
                class: 'arp025-chart-rain-bar ' + rainVisualClass(value),
                tabindex: 0,
                'aria-label': rainTooltip,
                'data-arp025-tooltip': rainTooltip
            });
            svg.appendChild(bar);
        });
        ...
        if (cleanPoints.length) {
            svg.appendChild(svgElement('path', {
                ...
                class: 'arp025-chart-line arp025-series-rain-cumulative',
                fill: 'none'
            }));
            cumulativePoints.forEach(function (point, index) {
                ...
                var hitPoint = svgElement('circle', {
                    ...
                    cy: point.y,
                    r: 11,
                    class: 'arp025-chart-hitpoint arp025-series-rain-cumulative',
                    tabindex: 0,
                    'aria-label': cumulativeTooltip,
                    'data-arp025-tooltip': cumulativeTooltip,
                    fill: 'transparent',
                    stroke: 'none',
                    opacity: '0'
                });
        ...
        legend.appendChild(svgElement('rect', {
            ...
            height: 11,
            rx: 4,
            class: 'arp025-chart-rain-bar arp025-chart-rain-light'
        }));
        legend.appendChild(svgElement('text', {
            x: margin.left + 28,
            y: 21,
    ...
    function initApp(app) {
        ...
        var hours = Math.max(1, Math.min(102, parseInt(app.dataset.hours || '102', 10)));
        var timezone = app.dataset.timezone || 'Europe/Paris';
        var titlePrefix = app.dataset.titlePrefix || 'Prévisions ARPEGE';

        var input = app.querySelector('.arp025-city-input');
        var locateButton = app.querySelector('[data-arp025-locate]');
        var results = app.querySelector('.arp025-search-results');
        var status = app.querySelector('.arp025-search-status');
        var generalBody = app.querySelector('[data-arp025-body-general]');
        var stormBody = app.querySelector('[data-arp025-body-storms]');
        var stormSummary = app.querySelector('[data-arp025-storm-summary]');
        var snowBody = app.querySelector('[data-arp025-body-snow]');
        var snowSummary = app.querySelector('[data-arp025-snow-summary]');
        var tabs = Array.prototype.slice.call(app.querySelectorAll('[data-arp025-tab]'));
        var panels = Array.prototype.slice.call(app.querySelectorAll('[data-arp025-panel]'));
        var title = app.querySelector('[data-arp025-title]');
        var altitudeLine = app.querySelector('[data-arp025-altitude]');
        var meta = app.querySelector('[data-arp025-meta]');
        var generated = app.querySelector('[data-arp025-generated]');
        var stale = app.querySelector('[data-arp025-stale]');
        var stormTopScroll = app.querySelector('[data-arp025-top-scroll="storms"]');
        var stormScrollWrap = app.querySelector('[data-arp025-scroll-wrap="storms"]');
        var stormTable = app.querySelector('.arp025-storm-table');
        var snowTopScroll = app.querySelector('[data-arp025-top-scroll="snow"]');
        var snowScrollWrap = app.querySelector('[data-arp025-scroll-wrap="snow"]');
        var snowTable = app.querySelector('.arp025-snow-table');
        var chartTemperature = app.querySelector('[data-arp025-chart-temperature]');
        var chartPressure = app.querySelector('[data-arp025-chart-pressure]');
        var chartRain = app.querySelector('[data-arp025-chart-rain]');
        var chartWind = app.querySelector('[data-arp025-chart-wind]');
        var chartTitleTemperature = app.querySelector('[data-arp025-chart-title-temperature]');
        var chartTitlePressure = app.querySelector('[data-arp025-chart-title-pressure]');
        var chartTitleRain = app.querySelector('[data-arp025-chart-title-rain]');
        var chartTitleWind = app.querySelector('[data-arp025-chart-title-wind]');
        var rainTotal = app.querySelector('[data-arp025-rain-total]');
        ...
        function makeDayCell(date, rowSpan) {
            var cell = createCell('th', 'Date', 'arp025-day-cell arp025-day-' + weekdayToken(date));
            cell.scope = 'rowgroup';
            cell.rowSpan = rowSpan;
            cell.appendChild(textSpan(weekdayFormat.format(date), 'arp025-day-weekday'));
            cell.appendChild(textSpan(shortDateFormat.format(date), 'arp025-day-date'));
            return cell;
        }
        ...
        function setStatus(message, error) {
            ...
            status.classList.toggle('arp025-search-error', Boolean(error));
        }
        ...
        function setActiveView(view) {
            ...
            if (view === 'map' && selectedMapFocus) {
                window.requestAnimationFrame(function () {
                    var mapApp = app.querySelector('[data-arp025m-app]');
                    if (mapApp) {
                        mapApp.dispatchEvent(new CustomEvent('arp025m:focus-location', {
        ...
        function putMessage(body, message, error, colspan) {
            ...
            var cell = createCell('td', '', error ? 'arp025-loading arp025-load-error' : 'arp025-loading');
        ...
        function showTableMessage(message, error) {
            ...
            if (stormSummary) {
                ...
                stormSummary.classList.toggle('arp025-storm-summary-warning', Boolean(error));
            }
            if (snowSummary) {
                snowSummary.textContent = message;
                snowSummary.classList.toggle('arp025-snow-summary-warning', Boolean(error));
            }
        }
        ...
        function renderForecast(departmentData, commune) {
            ...
            function appendHazard(row, label, code) {
                ...
                var cell = createCell('td', label, numeric === null ? '' : 'arp025-hazard arp025-hazard-' + numeric);
            ...
            forecasts.forEach(function (step, currentIndex) {
                ...
                if (dayKey !== previousGeneralDay) {
                    row.classList.add('arp025-new-day');
                    row.appendChild(makeDayCell(date, dayCounts[dayKey]));
                    previousGeneralDay = dayKey;
                }

                var hourCell = createCell('td', 'Heure', 'arp025-hour arp025-hour-' + weekdayToken(date));
                ...
                var conditionCell = createCell('td', 'Temps', 'arp025-condition');
                conditionCell.appendChild(textSpan(condition.icon, 'arp025-icon'));
                conditionCell.appendChild(textSpan(condition.label));
                row.appendChild(conditionCell);

                var temperatureCell = createCell('td', 'Température', 'arp025-temperature ' + temperatureClass(temp));
                ...
                var windCell = createCell('td', 'Vent', 'arp025-wind-cell ' + windForceClass(windDisplay));
                ...
                if (direction) {
                    ...
                    directionBadge.className = 'arp025-direction ' + windForceClass(windDisplay);
                    directionBadge.title = 'Vent de ' + direction + ' • ' + formatNumber(directionDegrees, 0) + '°';
                    var arrow = textSpan('➜', 'arp025-wind-arrow');
                    arrow.style.transform = 'rotate(' + ((Number(directionDegrees) + 180) % 360) + 'deg)';
                    directionBadge.appendChild(arrow);
                    directionBadge.appendChild(textSpan(direction, 'arp025-wind-label'));
                ...
                if (dayKey !== previousStormDay) {
                    stormRow.classList.add('arp025-new-day');
                    stormRow.appendChild(makeDayCell(date, dayCounts[dayKey]));
                    previousStormDay = dayKey;
                }
                var stormHour = createCell('td', 'Heure', 'arp025-hour arp025-hour-' + weekdayToken(date));
                ...
                var thunderCell = createCell('td', 'Risque orage', thunder ? 'arp025-thunder-risk arp025-thunder-' + Number(thunderCode) : '');
                if (thunder) {
                    thunderCell.appendChild(textSpan(thunder.icon + ' ', 'arp025-risk-icon'));
                    thunderCell.appendChild(textSpan(thunder.label, 'arp025-risk-label'));
                } else {
                ...
                var capeCell = createCell('td', 'CAPE', finite(cape) && cape >= 1500 ? 'arp025-cape-high' : (finite(cape) && cape >= 500 ? 'arp025-cape-mid' : ''));
                ...
                appendNumber(stormRow, 'Foudre', lightning, 0, '/100', finite(lightning) && lightning >= 60 ? 'arp025-lightning-high' : '');
                ...
                var stormTypeCell = createCell('td', 'Type d’orage', 'arp025-storm-type');
                ...
                var detailsCell = createCell('td', 'Détails', 'arp025-details-cell');
                var detailsButton = document.createElement('button');
                detailsButton.type = 'button';
                detailsButton.className = 'arp025-details-button';
                detailsButton.textContent = 'Afficher';
                detailsButton.setAttribute('aria-expanded', 'false');
                detailsCell.appendChild(detailsButton);

                var advanced = document.createElement('div');
                advanced.className = 'arp025-advanced-list arp025-details-inline';
                ...
                [
                    ...
                    box.className = 'arp025-advanced-item';
                    box.appendChild(textSpan(item[0], 'arp025-advanced-label'));
                    box.appendChild(textSpan(finite(item[1]) ? formatNumber(item[1], item[2]) + item[3] : '—', 'arp025-advanced-value'));
                ...
                if (dayKey !== previousSnowDay) {
                    snowRow.classList.add('arp025-new-day');
                    snowRow.appendChild(makeDayCell(date, dayCounts[dayKey]));
                    previousSnowDay = dayKey;
                }
                var snowHour = createCell('td', 'Heure', 'arp025-hour arp025-hour-' + weekdayToken(date));
                ...
                var snowRiskCell = createCell('td', 'Risque neige', snowRisk ? 'arp025-snow-risk arp025-snow-' + Number(snowRiskCode) : '');
                if (snowRisk) {
                    snowRiskCell.appendChild(textSpan(snowRisk.icon + ' ', 'arp025-risk-icon'));
                    snowRiskCell.appendChild(textSpan(snowRisk.label, 'arp025-risk-label'));
                ...
                var stickCell = createCell('td', 'Tenue', finite(stickCode) ? 'arp025-stick arp025-stick-' + Number(stickCode) : '');
                ...
                var snowWindCell = createCell('td', 'Vent moyen / rafales', 'arp025-snow-wind ' + windForceClass(gustDisplay));
                ...
                var snowDetailsCell = createCell('td', 'Détails', 'arp025-details-cell arp025-snow-details-cell');
                var snowDetailsButton = document.createElement('button');
                snowDetailsButton.type = 'button';
                snowDetailsButton.className = 'arp025-details-button';
                ...
                snowAdvanced.className = 'arp025-snow-profile arp025-details-inline';
                snowAdvanced.hidden = true;

                function appendProfileSection(titleText, items) {
                    var section = document.createElement('section');
                    section.className = 'arp025-profile-section';
                    section.appendChild(textSpan(titleText, 'arp025-profile-title'));
                    var list = document.createElement('div');
                    list.className = 'arp025-profile-list';
                    items.forEach(function (item) {
                        var profileItem = document.createElement('div');
                        profileItem.className = 'arp025-profile-item';
                        profileItem.appendChild(textSpan(item[0], 'arp025-profile-label'));
                        profileItem.appendChild(textSpan(finite(item[1]) ? formatNumber(item[1], item[2]) + item[3] : '—', 'arp025-profile-value'));
            ...
            if (stormSummary) {
                stormSummary.classList.toggle('arp025-storm-summary-warning', !hasStormColumns);
                if (!hasStormColumns) {
                    ...
                    stormSummary.textContent = 'Risque orageux maximal sur les ' + forecasts.length + ' prochaines échéances : ' + maxRisk.icon + ' ' + maxRisk.label + ' • MUCAPE ARPEGE 0,25°.';
                }
            }
            if (snowSummary) {
                snowSummary.classList.toggle('arp025-snow-summary-warning', !hasSnowColumns);
                if (!hasSnowColumns) {
                    ...
                    snowSummary.textContent = 'Risque de neige maximal sur les ' + forecasts.length + ' prochaines échéances : ' + maxSnow.icon + ' ' + maxSnow.label + ' • cumul neige ARPEGE 0,25°.';
                }
            }
            ...
            if (altitudeLine) {
                ...
                altitudeLine.classList.toggle('arp025-altitude-missing', !finite(altitude));
            }
            selectedMapFocus = {
            ...
            renderLineChart(chartTemperature, chartLabels, [{ label: 'Température', values: chartTemps, className: 'arp025-series-temperature' }], {
                decimals: 0,
                unit: '°C',
                fillArea: false,
            ...
            renderLineChart(chartPressure, chartLabels, [{ label: 'Pression', values: chartPressures, className: 'arp025-series-pressure' }], {
                decimals: 0,
                unit: 'hPa',
                fillArea: false,
                metricMode: 'pressure',
            ...
            renderLineChart(chartWind, chartLabels, [
                { label: 'Vent moyen', values: chartWindValues, className: 'arp025-series-wind' },
                { label: 'Rafales', values: chartGustValues, className: 'arp025-series-gust' }
            ], {
                decimals: 0,
                unit: 'km/h',
                forceZero: true,
        ...
        function displaySearchResults(candidates) {
            ...
            candidates.forEach(function (candidate, position) {
                ...
                button.type = 'button';
                button.className = 'arp025-search-result';
                button.id = input.id + '-option-' + position;
                button.setAttribute('role', 'option');
                button.setAttribute('aria-selected', 'false');

                var name = textSpan(candidate.nom, 'arp025-result-name');
        ...
        if (window.ARP025_AUTOHEAL && window.ARP025_AUTOHEAL.url) {
            fetch(window.ARP025_AUTOHEAL.url, { method: 'POST', cache: 'no-store' }).catch(function () {});
        }
    ...
    whenReady(function () {
        document.querySelectorAll('[data-arp025-app]').forEach(initApp);
    });
}());
