#!/usr/bin/env python3
"""Construit les cartes et prévisions ARPEGE 0,25° (grille GLOB025) pour la France.
...
ARPEGE native (GLOB025, 0,25°) et non depuis les seules coordonnées des communes.
...
DATASET_API = (
    ...
    "paquets-arpege-resolution-0-25deg/"
)
DATASET_PAGE = "https://www.data.gouv.fr/datasets/paquets-arpege-resolution-0-25deg"
DEFAULT_CURRENT_METADATA_URL = (
    "https://raw.githubusercontent.com/alertesmeteo-hub/"
    "ARPEGE-0.25/data/index.json"
)
USER_AGENT = "alertes-meteo.com/arpege-025/1.0"
...
# Grille globale GLOB025 documentée par Météo-France : 90°N–90°S,
# longitudes 0–359,75°, pas régulier de 0,25°, soit 1440 × 721 points.
LEGACY_GRID = GridSpec(ni=1440, nj=721, lat_first=90.0, lon_first=0.0, step=0.25)
...
RESOURCE_RE = re.compile(
    r"^arpege__025__(?P<group>SP1|SP2|HP1|HP2|IP1|IP2|IP3|IP4)__"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "ARPEGE de Météo-France : la grille GLOB025 0,25° est publiée "
            "de +0 à +48 h par pas horaire, puis de +51 à +102 h par pas "
            "de 3 h sur les quatre réseaux quotidiens)"
        ),
    )
    parser.add_argument(
        "--resource-directory",
...
def grid_index(
    ...
    if grid.ni * grid.step >= 359.9 and normalized_longitude < grid.lon_first:
        normalized_longitude += 360.0
    column = int(round((normalized_longitude - grid.lon_first) / grid.step))
    row = max(0, min(grid.nj - 1, row))
    column = column % grid.ni if grid.ni * grid.step >= 359.9 else max(0, min(grid.ni - 1, column))
    index = row * grid.ni + column
    model_latitude = grid.lat_first - row * grid.step
    model_longitude = grid.lon_first + column * grid.step
    if model_longitude > 180.0:
        model_longitude -= 360.0
...
def load_catalog(path: Path, grid: GridSpec = LEGACY_GRID) -> NationalCatalog:
    ...
    return NationalCatalog(
        version=f"{payload.get('catalog_version', '1')}-arpege025",
...
def api_resources(session: requests.Session) -> list[Resource]:
    ...
    if not resources:
        raise RuntimeError("Aucune ressource ARPEGE 0,25° trouvée sur data.gouv.fr")
    return resources
...
def choose_resources(
    ...
    # GLOB025 : la v1.0.0 traite SP1/SP2 uniquement. Les paquets IP1 globaux
    # dépassent plusieurs gigaoctets chacun et ne sont pas nécessaires aux
    # prévisions communales de surface.
...
class MapSampler:
    ...
    def __init__(self, width: int, height: int, grid: GridSpec = LEGACY_GRID) -> None:
        ...
        normalized_longitudes = np.where(
            (grid.ni * grid.step >= 359.9) & (target_longitudes < grid.lon_first),
            target_longitudes + 360.0,
            target_longitudes,
        )
        columns = (normalized_longitudes - grid.lon_first) / grid.step
    ...
    *,
    ...
    model = {
        ...
        "dataset": (
            ...
            if is_api_source
            else "Paquets ARPEGE résolution 0,25°"
        ),
        "domain": "EURAT" if is_api_source else "GLOB025",
